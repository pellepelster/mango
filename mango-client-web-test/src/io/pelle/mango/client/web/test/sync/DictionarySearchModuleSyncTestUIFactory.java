...
public class DictionarySearchModuleSyncTestUIFactory extends BaseModuleUIFactory<Object, DictionarySearchModuleSyncTestUI> {

	public DictionarySearchModuleSyncTestUIFactory() {
		super(new String[] { DictionarySearchModule.SEARCH_UI_MODULE_ID });
	}

	@Override
	public void getNewInstance(final String moduleUrl, final AsyncCallback<DictionarySearchModuleSyncTestUI> moduleCallback, Map<String, Object> parameters, final Optional<IModuleUI<?, ?>> previousModuleUI) {
		ModuleHandler.getInstance().startModule(ModuleUtils.concatenate(moduleUrl, DictionarySearchModule.MODULE_LOCATOR), parameters, new BaseErrorAsyncCallback<IModule>() {
			...
			public void onSuccess(IModule result) {
				if (supports(moduleUrl, DictionarySearchModule.MODULE_ID)) {
					moduleCallback.onSuccess(new DictionarySearchModuleSyncTestUI((DictionarySearchModule) result));
				}
			}
		});
	}
