...
public class DictionaryEditorModuleSyncTestUIFactory extends BaseModuleUIFactory<Object, DictionaryEditorModuleSyncTestUI> {

	public DictionaryEditorModuleSyncTestUIFactory() {
		super(new String[] { DictionaryEditorModule.EDITOR_UI_MODULE_ID });
	}

	@Override
	public void getNewInstance(final String moduleUrl, final AsyncCallback<DictionaryEditorModuleSyncTestUI> moduleCallback, Map<String, Object> parameters, final Optional<IModuleUI<?, ?>> previousModuleUI) {
		ModuleHandler.getInstance().startModule(ModuleUtils.concatenate(moduleUrl, DictionaryEditorModule.MODULE_LOCATOR), parameters, new BaseErrorAsyncCallback<IModule>() {
			...
			public void onSuccess(IModule result) {
				if (supports(moduleUrl, DictionaryEditorModule.MODULE_ID)) {
					moduleCallback.onSuccess(new DictionaryEditorModuleSyncTestUI((DictionaryEditorModule) result));
				}
			}
		});
	}
